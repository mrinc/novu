...
import { CompileTemplate } from './compile-webhook-template.usecase';
import { CompileWebhookTemplateCommand } from './compile-webhook-template.command';
...
describe('Compile Template', function () {
  ...
  it('should render custom html', async function () {
    const result = await useCase.execute(
      CompileWebhookTemplateCommand.create({
        data: {
          branding: {
            color: '#e7e7e7e9',
          },
